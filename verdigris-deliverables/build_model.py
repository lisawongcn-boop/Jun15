...
lines = [
    ...
    "2. Headcount — Traditional — 37-person team by function, salary, bonus, fully loaded cost.",
...
rows = [
    ...
    ("Applications assessed per completed loan", 4, NUM1, "x", "Founder estimate: pipeline of 15–25 live cases vs ~2 completions/month per analyst → roughly 1 in 4 assessed cases completes.", False),
    ...
    ("Underwriter time per assessed case (traditional)", 12, NUM1, "hours", "Founder estimate: 15–25 live cases, ~2 completions/month, ~50% of week on document work. Includes reading, reconciling, chasing, credit paper.", True),
    ("Working hours per FTE per year", 1650, NUM, "hours", "37.5 h/week × 44 productive weeks.", False),
    ("UNDERWRITING CAPACITY", None, None, None, None, False),
    ("Live cases one underwriter carries today (low)", 15, NUM, "cases", "Founder: each analyst looks after 15–25 live deals at once. Completion time is set by valuers, monitoring surveyors and solicitors, so capacity, not speed, is the constraint.", False),
    ("Live cases one underwriter carries today (high)", 25, NUM, "cases", "Founder: 15–25 live deals per analyst.", True),
]
r = 5
addr = {}
...
LIVE_LO = addr["Live cases one underwriter carries today (low)"]
LIVE_HI = addr["Live cases one underwriter carries today (high)"]
...
T, T_tot, T_off, T_soft, T_first, T_last = headcount_sheet(
    ...
    "Northgate Bridging (composite) — 37-person traditional lender",
    trad_team,
...
for lab, f1, f2, fmt in cmp_rows:
    ...
    r += 1
C.cell(row=6, column=6, value="Traditional: 37 people. AI-native: 8 people, AI does reading/reconciling/drafting.").font = NOTE
...
pc = [
    ...
    ("", None, None, None, ""),
    ("Capacity multiple per underwriter", None, "=C6/D6", X, "Traditional hours per case ÷ AI-native review hours per case. Agents hold the file while it waits on third parties."),
    ("Live cases one underwriter can carry (low)", f"={LIVE_LO}", "=C17*$D$16", NUM, "Today's caseload × capacity multiple."),
    ("Live cases one underwriter can carry (high)", f"={LIVE_HI}", "=C18*$D$16", NUM, "The headline: 15–25 live deals today → 120–200 with agents. Beyond 200 needs review time below 1.5 h; not assumed."),
]
r = 5
...
src = [
    ...
    ("Founder inputs", "Lisa Huang — underwriting experience: ~25 loans/yr, 15–25 live cases, £500k–£1m facilities, 15–28 day completions, broker fee 1–2%"),
...
import os
out = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Verdigris_Economics_Model.xlsx")
wb.save(out)
print("saved", out)
